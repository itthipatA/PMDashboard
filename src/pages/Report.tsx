...
import { PickersDayProps } from '@mui/x-date-pickers/PickersDay'; // Import PickersDayProps
...
const Report: React.FC<ReportProps> = ({ isDarkMode = false }) => {
  // Receive and default isDarkMode prop
  ...
  const filteredSensors = dummySensors.filter((s) =>
    s.toLowerCase().includes(sensorSearchTerm.toLowerCase()),
  );
  ...
  const handleToggleGroup = (value: string) => () => {
    ...
    if (currentIndex === -1) newSelected.push(value);
  ...
  const handleDateChange = (newValue: Dayjs | null) => {
    ...
    if (
      ...
      if (today.isSameOrAfter(currentStart)) {
    ...
    <div className="report-page-container">
      ...
      <Paper elevation={3} className="selection-panel group-panel">
        ...
        <List dense className="selection-list">
          <ListItem divider className="list-header">
            Group
          </ListItem>
          {filteredGroups.map((group) => (
            <ListItem key={group} disablePadding>
              <ListItemButton
                ...
                dense
                sx={{
                  // Apply sx to ListItemButton instead
                  color: isDarkMode ? 'var(--text-dark)' : 'var(--text-light)',
                  '.MuiListItemText-primary': {
                    // Target inner text explicitly if needed
                    color: isDarkMode
                      ? 'var(--text-dark)'
                      : 'var(--text-light)',
                  },
          ...
          {filteredSensors.map((sensor) => (
            <ListItem key={sensor} disablePadding>
              ...
              >
                <Checkbox
                  edge="start"
                  checked={selectedSensors.indexOf(sensor) !== -1}
                  tabIndex={-1}
                  disableRipple
                  size="small"
                />
            ...
            <DateCalendar
              ...
              sx={{
                ...
                '.MuiDayCalendar-weekDayLabel': {
                  // Use MuiDayCalendar-weekDayLabel class
      ...
      <Paper elevation={3} className="report-preview-panel">
        ...
        <div className="report-details">
          <p>
            ...
            {selectedGroups.join(', ') || 'None'}
          </p>
          <p>
            <strong>Selected Sensors:</strong>{' '}
            {selectedSensors.join(', ') || 'None'}
          </p>
          {/* Display formatted dates from state */}
          <p>
            <strong>Date Range:</strong> {formatDate(startDate)} -{' '}
            {formatDate(endDate)}
          </p>
          <p>
            <em>(Report content will be generated here based on selections)</em>
          </p>
        </div>
