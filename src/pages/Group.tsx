...
const dummyGroups = Array.from(
  { length: 25 },
  (_, i) => `หมู่บ้านตัวอย่าง ${String.fromCharCode(65 + i)}`,
);
const dummySensors = [
  'All',
  'ซอยสุขสบาย',
...
const dummyDevices = Array.from({ length: 20 }, (_, i) => ({
  ...
  temp: (Math.random() * 5 + 25).toFixed(1), // temp is string here
  battery: Math.floor(Math.random() * 50) + 50,
  status: Math.random() > 0.2 ? 'good' : 'unhealthy', // status can be 'good' or 'unhealthy'
...
const Group: React.FC = () => {
  ...
  const filteredSensors = dummySensors.filter((s) =>
    s.toLowerCase().includes(sensorSearch.toLowerCase()),
  );
  ...
  const mapMarkers = displayedDevices
    .map((device) => {
    ...
    <div className="group-page-container">
              ...
              {displayedDevices.map((device) => (
                <li key={device.id} className="device-summary-item">
                  <span
                    ...
                    aria-label={device.status === 'good' ? 'Device good' : 'Device unhealthy'}
                  ></span>
                  ...
                  <div className="device-readings">
                    <span className="device-temp">
                      <ThermostatIcon fontSize="inherit" /> {device.temp}°C
                    </span>
                    <span className="device-battery">
                      <BatteryIcon fontSize="inherit" /> {device.battery}%
                    </span>
                  </div>
                </li>
              ))}
      ...
      <Paper elevation={2} className="panel summary-panel">
        ...
        <Box className="summary-cards-group">
          <SummaryValueCard
            title="Maximum"
            value={summaryTemp.max}
            unit="°C"
            colorClass="max-temp"
          />
          <SummaryValueCard
            title="Average"
            value={summaryTemp.avg}
            unit="°C"
            colorClass="avg-temp"
          />
          <SummaryValueCard
            title="Minimum"
            value={summaryTemp.min}
            unit="°C"
            colorClass="min-temp"
          />
        </Box>
